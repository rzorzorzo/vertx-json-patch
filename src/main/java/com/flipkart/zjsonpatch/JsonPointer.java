 ...
 * <p>
 * </p>
 * Generally, a JSON Pointer is a string representation of a path into a JSON
 * document. This class implements the RFC as closely as possible, and offers
 * several helpers and utility methods on top of it:
 *
 * <pre>
 ...
 *      Object doc = om.readTree("{\"foo\":[\"bar\", \"baz\"]}");
 *      Object baz = JsonPointer.parse("/foo/1").{@link #evaluate(Object) evaluate}(doc);
 *      assert(baz.textValue().equals("baz"));
 * </pre>
 *
 * <p>
 * Instances of {@link JsonPointer} and its constituent {@link RefToken}s are
 * <b>immutable</b>.
 *
 * @since 0.4.8
 */
class JsonPointer
{
	 ...
	 *
	 * @param tokens
	 *            The list of reference tokens from which to construct the new
	 *            pointer. This list is not modified.
	 */
	 ...
	 * @return An instance of {@link JsonPointer} conforming to the specified string
	 *         representation.
	 * @throws IllegalArgumentException
	 *             The specified JSON Pointer is invalid.
	 */
	 ...
	 * 
	 * @return {@code true} if this pointer represents the root node, {@code false}
	 *         otherwise.
	 */
	...
	/**
	 * Creates a new JSON pointer to the specified field of the object referenced by
	 * this instance.
	 *
	 * @param field
	 *            The desired field name, or any valid JSON Pointer reference token
	...
	/**
	 * Creates a new JSON pointer to an indexed element of the array referenced by
	 * this instance.
	 *
	 * @param index
	 *            The desired index, or {@link #LAST_INDEX} to point past the end of
	 *            the array.
	 * @return The new {@link JsonPointer} instance.
	 */
	 ...
	 *
	 * @return An <a href="https://tools.ietf.org/html/rfc6901#section-5">RFC 6901
	 *         compliant</a> string representation of this JSON pointer.
	 */
	 ...
	 *
	 * @return A list of {@link RefToken}s. Modifications to this list do not affect
	 *         this instance.
	 */
	 ...
	 *
	 * @param index
	 *            The desired reference token index.
	 * @return The specified instance of {@link RefToken}.
	 * @throws IndexOutOfBoundsException
	 *             The specified index is illegal.
	 */
	 ...
	 *             Last cannot be called on {@link #ROOT root} pointers.
	 */
	...
	/**
	 * Creates a JSON pointer to the parent of the node represented by this
	 * instance.
	 *
	...
	{
		throw new JsonPointerEvaluationException(message, new JsonPointer(Arrays.copyOf(tokens, atToken)), document);
	}
	 ...
	 * <a href="https://tools.ietf.org/html/rfc6901#section-4">RFC 6901 sectino
	 * 4</a>.
	 *
	 * @param document
	 *            The target document against which to evaluate the JSON pointer.
	 * @return The {@link Object} resolved by evaluating this JSON pointer.
	 * @throws JsonPointerEvaluationException
	 *             The pointer could not be evaluated.
	 */
	public Object evaluate(final Object document) throws JsonPointerEvaluationException
	{
		...
		for (int idx = 0; idx < tokens.length; ++idx)
		{
			...
			if ("array".equals(InternalUtils.getNodeType(current)))
			{
				if (!token.isArrayIndex())
					error(idx, "Can't reference field \"" + token.getField() + "\" on array", document);
				if (token.getIndex() == LAST_INDEX || token.getIndex() >= ((JsonArray) current).size())
					error(idx, "Array index " + token.toString() + " is out of bounds", document);
				current = ((JsonArray) current).getValue(token.getIndex());
			} else if ("object".equals(InternalUtils.getNodeType(current)))
			{
				if (!((JsonObject) current).containsKey(token.getField()))
					error(idx, "Missing field \"" + token.getField() + "\"", document);
				current = ((JsonObject) current).getValue(token.getField());
			} else
	 ...
	 *
	 * Such an index is represented by the JSON pointer reference token "{@code -}";
	 * see <a href="https://tools.ietf.org/html/rfc6901#section-4">RFC 6901 section
